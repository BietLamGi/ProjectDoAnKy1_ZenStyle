...
->orderBy('StartTime', 'desc')
    ...
    public function update(Request $request, Appointment $appointment)
    {
        ...
        // Checking in means a staff member is assigned and the appointment
        // date has arrived - check against the values being submitted right
        // now, since staff/date can be changed in this same form submit.
        if ($data['Status'] === 'CheckedIn' && $appointment->Status !== 'CheckedIn') {
            if (!$data['StaffID']) {
                return back()->withErrors([
                    'Status' => 'Assign a staff member before checking the customer in.',
                ])->withInput();
            }

            if (Carbon::parse($data['AppointmentDate'])->startOfDay()->gt(Carbon::today())) {
                return back()->withErrors([
                    'Status' => 'This appointment is scheduled for a future date and cannot be checked in yet.',
                ])->withInput();
            }
        }
...
return redirect()
    ...
    public function updateStatus(Request $request, Appointment $appointment)
    {
        ...
        if ($request->status === 'CheckedIn') {
            $checkInError = $this->checkInBlockedReason($appointment);
            if ($checkInError) {
                return back()->with('error', $checkInError);
            }
        }
    ...
    /**
     * Why an appointment can't be checked in right now, or null if it can.
     * Checking in means the customer has physically arrived and a staff
     * member is about to serve them - both must already be true:
     *   1. A staff member must be assigned (can't serve with no one assigned).
     *   2. The appointment date must have arrived (can't check in a customer
     *      in for a future-dated appointment).
     */
    private function checkInBlockedReason(Appointment $appointment): ?string
    {
        if (!$appointment->StaffID) {
            return 'Assign a staff member to appointment #' . $appointment->AppointmentID . ' before checking the customer in.';
        }

        if (Carbon::parse($appointment->AppointmentDate)->startOfDay()->gt(Carbon::today())) {
            return 'Appointment #' . $appointment->AppointmentID . ' is scheduled for a future date and cannot be checked in yet.';
        }
