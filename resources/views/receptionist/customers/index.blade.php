...
<div class="container-fluid">
    ...
    <div class="page-heading">
        <div>
            ...
            <h1>Khách hàng</h1>
            <p class="text-muted mb-0">Quản lý hồ sơ khách hàng: tìm kiếm, thêm mới, cập nhật thông tin.</p>
        </div>
    ...
    <div class="panel">
        <div class="panel-header">
            <form class="d-flex" method="GET">
                <input type="search" name="q" value="{{ $keyword }}" class="form-control table-search" placeholder="Tìm theo tên, SĐT, email...">
                    ...
                    <tbody>
                        @foreach ($customers as $customer)
                            <tr>
                                ...
                                <td class="text-end">
                                    <a href="{{ route('receptionist.appointments.create', ['customer_id' => $customer->CustomerID]) }}" class="btn btn-sm btn-light" title="Đặt lịch cho khách">
                                        <i class="bi bi-calendar-plus"></i>
                                    </a>
                                    <a href="{{ route('receptionist.customers.edit', $customer) }}" class="btn btn-sm btn-light" title="Sửa">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{ route('receptionist.customers.destroy', $customer) }}" method="POST" class="d-inline" onsubmit="return confirm('Xoá khách hàng này?');">
                                        @csrf
                                        @method('DELETE')
