                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            ...
                            ?? [];
                            // "CheckedIn" also needs a staff assigned and the
                            // appointment date to have arrived - hide it from
                            // the quick menu until both are true (server still
                            // enforces this either way).
                            $canCheckIn = $appointment->StaffID
                            && \Illuminate\Support\Carbon::parse($appointment->AppointmentDate)->startOfDay()->lte(\Illuminate\Support\Carbon::today());
                            $allowedNext = collect($allowedNext)->reject(fn ($s) => $s === 'CheckedIn' && !$canCheckIn)->values()->all();
                            @endphp
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <li>
                                        <form action="{{ route('receptionist.appointments.status', $appointment) }}"
                                            method="POST"
                                            onsubmit="return confirm('{{ $s === 'CheckedIn' ? 'Confirm the customer has actually arrived and is being served? This will allow the appointment to be invoiced/paid.' : 'Move appointment #' . $appointment->AppointmentID . ' to \'' . $s . '\'?' }}');">
                                            @csrf
                                            @method('PATCH')
                            ...
                            @elseif (!$isLocked && in_array('CheckedIn', \App\Http\Controllers\Receptionist\AppointmentController::ALLOWED_TRANSITIONS[$appointment->Status] ?? [], true) && !$canCheckIn)
                            <span class="text-muted small" title="Assign a staff member and wait until the appointment date to check this customer in.">
                                @if (!$appointment->StaffID) No staff assigned @else Not yet due @endif
                            </span>
                            @endif
                            ...
                            @elseif ($appointment->Status === 'CheckedIn')
