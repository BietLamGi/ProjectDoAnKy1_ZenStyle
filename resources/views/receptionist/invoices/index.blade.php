    ...
    <div class="panel">
        <div class="panel-header">
            <form class="d-flex flex-wrap gap-2 align-items-center" method="GET">
                ...
                @if ($onlyUnconfirmed)
                    <a href="{{ route('receptionist.invoices.index', ['q' => $keyword]) }}" class="btn btn-outline-secondary">
                        <i class="bi bi-list-ul"></i> Show all invoices
                    </a>
                @else
                    <a href="{{ route('receptionist.invoices.index', ['q' => $keyword, 'unconfirmed' => 1]) }}" class="btn btn-outline-warning">
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        @if ($appointment->Status === 'CheckedIn')
                                            <a href="{{ route('receptionist.invoices.create', ['appointment_id' => $appointment->AppointmentID]) }}" class="btn btn-sm btn-primary">
                                                <i class="bi bi-plus-lg"></i> Create invoice
                                            </a>
                                        @else
                                            <span class="text-muted small" title="Customer must be checked in before this appointment can be invoiced.">
                                                Not checked in yet
                                            </span>
                                        @endif
                                    </td>
                    ...
                    <tbody>
                        @foreach ($invoices as $invoice)
                            <tr>
                                ...
                                <td class="text-end">
                                    <a href="{{ route('receptionist.invoices.show', $invoice) }}" class="btn btn-sm btn-light" title="View">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    @if ($invoice->PaymentMethod)
                                        <span class="badge text-bg-success">Paid</span>
                                    @else
                                        <a href="{{ route('receptionist.invoices.edit', $invoice) }}" class="btn btn-sm btn-warning" title="Record payment">
